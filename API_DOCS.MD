...
Payload (implementasi tidak memberi tag `required`, jadi semua field boleh kosong):

```json
{ "elevation": "120 mdpl", "coordinate": "-6.5561,107.4421", "population": "5.000 jiwa" }
```
...
```json
{
	...
	"data": { "elevation": "120 mdpl", "coordinate": "-6.5561,107.4421", "population": "5.000 jiwa" }
}
```
...
Response `200`: envelope dengan message `Maps retrieved successfully` dan `data` berupa array objek map (dengan `id`, `elevation`, `coordinate`, `hamlet_one`, `hamlet_two`, `population`, `is_active`).

### `POST /map/update` — Protected

Payload (`id` wajib; field lainnya boleh kosong):

```json
{ "id": 1, "elevation": "125 mdpl", "coordinate": "-6.5561,107.4421", "population": "5.100 jiwa" }
```
...
```json
{
	...
	"data": [{ "id": 1, "title": "Gotong Royong", "image": "uploads/gallery/image.jpg" }]
}
```
...
Wajib: `category_id > 0`, `title`, `slug`, `description`, `owner_name`. `subtitle`, `location_id`, `location`, `owner_msisdn`, `media_id` opsional. Jika `location` dikirim, latitude harus -90..90 dan longitude -180..180. `media_id` adalah base64/data URL atau `null`; `uploaded_by` berasal dari token.
...
Wajib: `name`, `province`, `regency`, `district`, `address`. Field lain opsional/default zero value. `headman` opsional; jika dikirim, `name` wajib dan `position` harus `kepala-desa` (string kosong otomatis menjadi nilai tersebut).

### `POST /profile/create` — Protected

Payload: payload profile di atas.
...
### `POST /profile/update` — Protected

Payload: payload profile di atas ditambah `"id": 1`; field wajib create tetap wajib pada update.

Response `200`: `{ "code": 200, "message": "Profile updated successfully" }`
