...
export const updatePotential = async (
	payload: UpdatePotentialPayload
): Promise<UpdatePotentialResponse> => {
	const { data } = await api.post<UpdatePotentialResponse>('/potential/update', payload);
	return data;
};

export const deletePotential = async (
	payload: DeletePotentialPayload
): Promise<DeletePotentialResponse> => {
